...
def get_mlc_metrics(preds, targets):
    ...
    METRICS = ['HA', 'ebF1', 'miF1', 'maF1', 'p_at_1', 'accuracy', 'precision', 'recall']
...
def compute_metrics(predictions, targets, threshold, all_metrics=True):
    ...
    precision = tp / (tp + fp + 1e-10)  # Add a small epsilon to avoid division by zero
    recall = tp / (tp + fn + 1e-10)     # Add a small epsilon to avoid division by zero
    ...
    metrics_dict['accuracy'] = ACC  # Already calculated as ACC
    metrics_dict['precision'] = macro_precision
    metrics_dict['recall'] = macro_recall
