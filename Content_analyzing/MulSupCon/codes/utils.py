...
from WF_Dataloader.Datamaker import DatasetMaker
...
def get_model_from_pretrain(
    ...
    **kwargs
    ...
    if model_path is None: # 创建一个新模型并返回
        model = getattr(models, config['model'])(**config['model_args'], **kwargs)
        return model, {}, {}
    pretrain_config = torch.load(
        glob(os.path.join(model_path, '*config*'))[0], map_location='cpu') # 从路径加载模型
    if resume: # 加载预训练模型配置
...
def get_transform(
    ...
    **kwargs
    ...
    if not is_image:
        ...
        if h5:
            scaler = StandardScaler()
            # with h5py.File(h5, 'r') as input_file:
            #     features = input_file['feature'][:]
            dataset = DatasetMaker(h5)
            features = dataset.features[:]
            # 展平每个样本为1D向量 (n_samples, total_features)
            n_samples, dim, length = features.shape
            features_reshaped = features.transpose(0, 2, 1).reshape(-1, dim)

            scaler.fit(features_reshaped)

            def _scaler_transform(x):
                # x 形状为 (dim, length)
                x_transposed = x.T  # (length, dim)
                x_scaled = scaler.transform(x_transposed)  # 标准化每个通道
                return x_scaled.T  # 恢复为 (dim, length)
        
        scaler_transform = lambda x: torch.from_numpy(_scaler_transform(x)).float()
...
'''
def _scaler_transform(x, scaler=None):
    if scaler is not None:
        return scaler.transform(x)
    return x  # 如果没有提供 scaler，返回输入

# 定义一个模块级别的函数，用于应用掩码
def _mask_transform(vector, p=0):
    if not p:
        return vector
    mask = torch.zeros_like(vector)
    mask.uniform_()
...
# 将转换函数封装为可以在外部调用的函数
def get_train_transform(scaler=None, p=0):
    def train_transform(x):
        x = torch.from_numpy(_scaler_transform(x, scaler))
        x = _mask_transform(x, p)
        return x
    return train_transform

def get_eval_transform(scaler=None):
    def eval_transform(x):
        return torch.from_numpy(_scaler_transform(x, scaler))
    return eval_transform

def get_transform(is_image: bool = True, h5: str = None, p: float = 0, **kwargs):
    scaler = None
    if not is_image and h5:
        scaler = StandardScaler()
        with h5py.File(h5, 'r') as input:
            features = input['feature'][:]
        scaler.fit(features)

    train_transform = get_train_transform(scaler, p)
    eval_transform = get_eval_transform(scaler)
