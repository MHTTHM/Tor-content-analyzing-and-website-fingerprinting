    ...
    def __init__(
        ...
        with torch.no_grad():
            if backbone == 'TransformerModel':
                x = torch.randn(4, backbone_kwargs['in_dim'], backbone_kwargs.get('in_dim', 512)) # 假设序列长度为10
            elif 'in_dim' in backbone_kwargs:
                x = torch.randn(256, 2, backbone_kwargs['in_dim'])
            else:
...
class PretrainModel(PretrainBase):
    def __init__(
        ...
        backbone_kwargs: dict = {},
        n_class: int = 80,
        output_func = lambda x: None, #根据模型输出生成最终的输出
        cap_Q: int = 2048,
    ...
    def __init__(
        ...
        with torch.no_grad():
            if 'in_dim' in backbone_kwargs:
                ...
                x = torch.randn(4, 2, backbone_kwargs['in_dim'])
            else:
                x = torch.randn(4, 3, 224, 224)
