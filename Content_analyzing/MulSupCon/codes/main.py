...
class Runner(object):
    ...
    def pretrain(
        ...
        **kwargs):
        ...
        if resume:
            ...
            dataset = DatasetMaker(config['train'])
            n_class = dataset.labels.shape[-1]
        ...
        model, optim_params, scheduler_params = utils.get_model_from_pretrain( # 在这里获取预训练模型
        ...
        model.cuda(0)
        # if config['criterion'] != 'WeightedSupCon' and config['criterion'] != 'SupCon':
        ...
        def _train(_, batch):
            ...
            with torch.enable_grad():
                score, mask = Runner._forward(model, batch) # get data
    ...
    def train(
        ...
        **kwargs
        ...
        sample_batch, sample_labels = next(iter(TrainDataloader))
        ...
        sample_batch, sample_labels = next(iter(DevDataloader))
        print(f"Dev Batch shape: {sample_batch.shape}")  
        ...
        sample_batch, sample_labels = next(iter(TestDataloader))
        print(f"Test Batch shape: {sample_batch.shape}")  
