...
class RandomSampler(Sampler):
    ...
    def get_batch(self, batch_indices):
        """
        Retrieve a batch of data from the h5 file using the batch_indices.
        
        Parameters:
            batch_indices (list): List of indices to retrieve the batch data.
        
        Returns:
            batch_data: The batch of data corresponding to the indices.
        """
        dataset = DatasetMaker(self.h5)
        data = dataset.features[batch_indices, :, :]
        # with h5py.File(self.h5, 'r') as input_data:
        #     data = input_data['data'][batch_indices, :, :]  # Assuming data is 3D: B x H x W
        return data
...
class TrainVectorDataset(Dataset):
    """
    Train Dataset for multidimensional data
    """
    def __init__(
            self,
            h5: str,
        ...
        self.transform = transform
        dataset = DatasetMaker(self.h5)
        self.feature = dataset.features[:].astype(np.float32)
        self.targets = dataset.labels[:]

        # with h5py.File(self.h5, 'r') as input:
        #     self.targets = input['target'][:]  # Assuming targets are 1D or 2D (one-hot)
        #     self.feature = input['feature'][:].astype(np.float32)  # Assuming feature is N-dimensional
        self.targets = torch.from_numpy(self.targets).to(torch.float32)

    def __getitem__(self, i):
        feature = self.feature[i]  # 原始形状，如(100, 2)
        target = self.targets[i]
        # 转换时自动展平并恢复形状
        view1 = self.transform(feature.copy())
        view2 = self.transform(feature.copy())
        return torch.stack([view1, view2], dim=0), target
...
class EvalVectorDataset(Dataset):
    """
    Evaluation Dataset for multidimensional data
    """
    def __init__(
            self,
            h5: str,
    ...
    def __getitem__(self, i):
        index = self.indices[i]
        feature = self.feature[index]  # Shape: (dim, length)
        target = self.targets[index]
        
        # # 直接应用 transform 到原始二维数据
        # view1 = self.transform(feature.copy())
        # view2 = self.transform(feature.copy())
