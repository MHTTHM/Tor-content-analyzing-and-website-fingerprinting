...
def run(para, times = 0):
    ...
    try:
        ...
        if times < 5:
            ...
            with open("error_info.txt", 'r+') as f:
                f.write("{}\t{}\n".format(ind, sentence))
            return None
    print(ind)
    return str(ind)+'\t'+l
...
if __name__ == "__main__":
    print('reading...')
    data = []
    with open("./data/{0}/{0}.txt".format(dataset), 'r', encoding='utf-8') as fin:
        for line in fin:
            ind, cate, content = line.split('\t')
            if content == None:   #default
                continue
    ...
    fout = open("./data/{0}/{0}2entity.txt".format(dataset), 'w', encoding='utf-8')
    fout.write('\n'.join(outdata))
    fout.close()
