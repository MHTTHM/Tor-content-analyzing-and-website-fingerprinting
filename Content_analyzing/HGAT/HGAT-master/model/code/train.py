...
sheet = book.add_sheet('隐藏服务非法网页数量统计',cell_overwrite_ok=True)
col = ('Counterfeit','Drugs','Hacking','Others','Porngraphy','Violence')
for i in range(0,6):
...
LR = 0.01 if dataset == 'snippets' else 0.005 #0.005
...
parser.add_argument('--epochs', type=int, default=2,  #修改轮次
...
def evaluate(preds_list, y_list):
    ...
    if multi_label:
        ...
        ER = accuracy_score(y_list, preds) * 100
        print('\n',"preds lenght",len(preds))
        if(len(preds)==21720):  #改为测试集数量
            #print('preds:',preds)
            for i in range(0,21720):
                data=preds[i]
                for j in range(0,6):
                    sheet.write(i+1,j,int(data[j]))
            savepath = r'J:\博士研究\零碎事情\带学生\朱怡霖\HGAT半监督分类\result.xls'
            book.save(savepath)
            print('book saved!')
...
def test(epoch, input_adj_test, input_features_test, idx_out_test, idx_test):
    ...
    if isinstance(output, list):
        ...
        #print('output_idx_test: ',output[0][idx_out_test],'\n')
        #print('labels:',labels[idx_test])
    else:
